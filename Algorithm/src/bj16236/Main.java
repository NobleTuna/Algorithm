...
//			System.out.println(targetY + " " + targetX + " " + tD);
	...
	static void searchFood(int y, int x) { // 목표로 할 먹이좌표
		...
		while (!q.isEmpty()) {
			...
			if (mx == x && my == y) { // 상어위치 도착하면 거리부터 비교
				if (tD > curM.distance) { // 거리가 더 가깝다면
					tD = curM.distance;
					targetX = x;
					targetY = y;
					return;
				} else if (tD == curM.distance) { // 거리가 같다면
					if (my < targetY) { // 좌표값이 현재가 더 높다면
						tD = curM.distance;
						targetX = x;
						targetY = y;
						return;
					} else if (my == targetY) { // 높이가 같다면
						if (mx < targetX) { // 현재가 더 왼쪽에 있다면
							tD = curM.distance;
							targetX = x;
							targetY = y;
							return;
