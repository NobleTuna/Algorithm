...
public class Solution {
	...
	static void goOne(int sy, int sx, boolean[] eat, int y, int x, int cnt) { // 왼쪽아래 기본
		int ny = y + 1;
		int nx = x + 1;
		if (!outRange(ny, nx) && !eat[map[ny][nx]]) { // 오른쪽 아래가 범위밖에 아니거나 먹지않았으면 진행
			eat[map[ny][nx]] = true;
			goOne(sy, sx, eat, ny, nx, cnt + 1);
			eat[map[ny][nx]] = false;
		}
		...
		ny = y + 1;
		nx = x - 1;
		if (!outRange(ny, nx) && !eat[map[ny][nx]]) { // 왼쪽아래로
			eat[map[ny][nx]] = true;
			goTwo(sy, sx, eat, ny, nx, cnt + 1);
			eat[map[ny][nx]] = false;
		}
	}
	...
	static void goTwo(int sy, int sx, boolean[] eat, int y, int x, int cnt) { // 왼쪽아래 기본
		int ny = y + 1;
		int nx = x - 1;
		if (!outRange(ny, nx) && !eat[map[ny][nx]]) { // 왼쪽아래 범위밖에 아니거나 먹지않았으면 진행
			eat[map[ny][nx]] = true;
			goTwo(sy, sx, eat, ny, nx, cnt + 1);
			eat[map[ny][nx]] = false;
		}
...
//		if (sy == 0 && sx == 1 && ny == 1 && nx == 0) {
////			System.out.println("aaa");
////			System.out.println(map[ny][nx]);
////			System.out.println(eat[map[ny][nx]]);
//		}

		if (!outRange(ny, nx) && !eat[map[ny][nx]]) { // 왼쪽위 범위밖에 아니거나 먹지않았으면 진행
			eat[map[ny][nx]] = true;
			goThree(sy, sx, eat, ny, nx, cnt + 1);
			eat[map[ny][nx]] = false;
		}
	}

	static void goThree(int sy, int sx, boolean[] eat, int y, int x, int cnt) { // 왼쪽위
//		if (sy == 0 && sx == 1 && y == 1 && x == 0)
...
//			System.out.println(sy+" "+sx);
		...
		if (!outRange(ny, nx) && !eat[map[ny][nx]]) { // 오른쪽위
			eat[map[ny][nx]] = true;
			goFour(sy, sx, eat, ny, nx, cnt + 1);
			eat[map[ny][nx]] = false;
		}
	}

	static void goFour(int sy, int sx, boolean[] eat, int y, int x, int cnt) { // 오른쪽 위 쭉
