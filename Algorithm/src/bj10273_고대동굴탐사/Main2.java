...
public class Main2 {
	...
	static int N, E;
	static int[] cave;
	static HashMap<String, Integer> connected;
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			cave = new int[N + 1];
			connected = new HashMap<String, Integer>();
			memory = new int[N + 1];
	...
	public static void go(int point, StringBuilder course, int value) {
		...
		for (int i = 1; i <= N; i++) {
			...
			if (!connected.containsKey(key)) {
				...
				go(i, new StringBuilder(course).append(String.valueOf(i)).append(" "),
						value + cave[i] - connected.get(key));
			}
		}
	}
